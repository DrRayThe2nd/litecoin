Pittscoin Core version *0.15.1* is now available from:

  <https://download.pittscoin.org/pittscoin-0.15.1/>
...
A number of changes to the way Pittscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 micros per kB (0.00001 PTS). This is to prepare for dropping the minimum transaction fee to 0.1 micros per kB (0.0001 PTS) in the next release.
