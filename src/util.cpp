...
const char * const BITCOIN_CONF_FILENAME = "pittscoin.conf";
const char * const BITCOIN_PID_FILENAME = "pittscoind.pid";
...
#else
    const char* pszModule = "pittscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pittscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pittscoin";
#else
    // Unix
    return pathRet / ".pittscoin";
#endif
#endif
}
